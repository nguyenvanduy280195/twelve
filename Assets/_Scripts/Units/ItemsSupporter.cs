...
public class ItemsSupporter
{
    ...
    public List<GameObject> GetMatchesInCol(int col, int row, string tag)
    {
        ...
        var itemsInBottom = _items.AsList.Where(go => go != null && go.GetComponent<Item>().col == col && go.GetComponent<Item>().row < row)
                                         .Distinct()
        ...
        var itemsInTop = _items.AsList.Where(go => go != null && go.GetComponent<Item>().col == col && row < go.GetComponent<Item>().row)
                                      .Distinct()
    ...
    public List<GameObject> GetMatchesInRow(int col, int row, string tag)
    {
        ...
        var itemsInLeft = _items.AsList.Where(go => go != null && go.GetComponent<Item>().row == row && go.GetComponent<Item>().col < col)
                                       .Distinct()
                                       .OrderByDescending(it => it.GetComponent<Item>().col);
        ...
        var itemsInRight = _items.AsList.Where(go => go != null && go.GetComponent<Item>().row == row && col < go.GetComponent<Item>().col)
                                        .Distinct()
    ...
    public IEnumerable<(ItemLocation, ItemLocation, int)> AllSwappableItems
    {
        get
        {
            ...
            {
                var key1 = new ItemLocation(gos.Item1.GetComponent<Item>().col, gos.Item1.GetComponent<Item>().row);
                var key2 = new ItemLocation(gos.Item2.GetComponent<Item>().col, gos.Item2.GetComponent<Item>().row);
                var key = (key1, key2);
